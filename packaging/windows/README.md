...
| `../../desktop/winres.json` | the window's resources: the same icon and strings, plus the application manifest the window toolkit wants (`desktop/build/windows/wails.exe.manifest`: per-monitor DPI awareness, common controls v6); `make-syso.sh` makes `desktop/rsrc_windows_amd64.syso` from it |
