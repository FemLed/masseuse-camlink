...
```
Masseuse.exe               the connector, byte for byte goreleaser's windows_amd64 masseuse-camlink.exe, then:
...
The first time it runs, the program unpacks the payload under its state
directory, `%LOCALAPPDATA%\masseuse-camlink\bin\<id>\` (`cmd/masseuse-camlink/payload.go`;
the id names the contents, so a new version unpacks beside the old one's
directory, which is then removed), checks every file against the
manifest, and looks for ffmpeg and the helpers there before anywhere else
(`internal/capture.BundledDir`, `helpersDir`). A start that finds the
files whole leaves them; a damaged or missing file is written again.
`Masseuse.exe --version` never unpacks anything.
...
the signature's description. `masseuse-camlink` stays the program's name
for engineers: the archives' binary, the state directory, the flags.

## What the program does on Windows

Opened from Explorer, `Masseuse.exe` is a console program with a window
...
checksum, both sides are stripped), and the package stripped must hash to
the published `windows_amd64` connector. `VERIFY.md`, "The Windows
...
| `pack/` | `go run ./packaging/windows/pack -v VERSION -b CONNECTOR -f FFMPEG_DIR [-u UNITS_DIR] -o OUT.exe` appends the payload to the connector and checks the result: the payload located and verified in the file written, and `internal/payload.Strip` giving the connector back. Plain Go, so it runs on the Windows release runner and the Linux CI runner alike; nothing is signed here |
...
checks the executable's resources, fetches and verifies the helpers, signs
ffmpeg and the helpers (when it can) and checks them with `pestrip`, packs
the payload, signs the package and checks it with `pestrip` and with
Windows' own verdict (`Get-AuthenticodeSignature`), runs the package from
a directory of its own (the payload unpacks, the helper is found, ffmpeg
runs), writes the zip for older installs, writes `checksums-windows.txt`,
...
go run ./packaging/windows/pack -v 0.0.0 -b dist/masseuse-camlink.exe -f dist/ffmpeg-windows -u dist/units -o dist/Masseuse.exe
go run ./cmd/pestrip -sha256 dist/Masseuse.exe    # the connector's hash
```
