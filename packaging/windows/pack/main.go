// Command pack assembles the Windows download, Masseuse.exe: the desktop
// window (desktop/, docs/DESKTOP.md) with the published windows_amd64
// connector, ffmpeg.exe, the unit driver helpers, the notices and a README
// appended as a payload (internal/payload), one file that is the whole
// program. The window unpacks the payload under the state directory when
// it runs and starts the connector from there. The connector's bytes are
// left as they are inside the payload, so cmd/pestrip -payload gives the
// published binary back byte for byte (minus the Authenticode signature
// the release puts on it first); the release workflow signs the
// connector, ffmpeg.exe and the helpers before handing them here, and
// signs the package after. Plain Go, so it runs on the Windows release
// runner and on the Linux CI runner alike; nothing is signed here.
//
// usage: go run ./packaging/windows/pack -v VERSION -s SHELL -b CONNECTOR -f FFMPEG_DIR [-u UNITS_DIR] -o OUT.exe [-repo DIR]
//
//	VERSION     the release version without the v (0.0.0 for a local build)
//	SHELL       the desktop window's Masseuse.exe (cd desktop && wails3 task
//	            build), unsigned: it becomes the file, the payload behind it
//	CONNECTOR   the masseuse-camlink.exe to pack (goreleaser's windows_amd64
//	            binary; signed or not), into the payload
...
func main() {
	...
	flag.StringVar(&o.shell, "s", "", "the desktop window's Masseuse.exe, the file the payload goes behind")
	...
	flag.Parse()
	if o.version == "" || o.shell == "" || o.connector == "" || o.ffmpegDir == "" || o.out == "" || flag.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "usage: pack -v VERSION -s SHELL -b CONNECTOR -f FFMPEG_DIR [-u UNITS_DIR] -o OUT.exe [-repo DIR]")
		os.Exit(2)
	}
	if o.repo == "" {
	...
	fmt.Printf("wrote %s (%d bytes); payload %s; the window is sha256 %s; the connector inside is sha256 %s\n", o.out, rep.size, rep.id, rep.shellSHA256, rep.connectorSHA256)
...
type options struct {
	version, shell, connector, ffmpegDir, unitsDir, out, repo string
}
...
// window again, byte for byte.
func build(o options) (*report, error) {
	if !versionOK(o.version) {
		return nil, fmt.Errorf("version %s is not X.Y.Z", o.version)
	}
	shell, err := os.ReadFile(o.shell)
	if err != nil {
		return nil, err
	}
	if h, err := pesig.ParseBytes(shell); err != nil {
		return nil, fmt.Errorf("%s: %w", o.shell, err)
	} else if h.Signed() {
		return nil, fmt.Errorf("%s is signed; the package is signed after packing, the window inside never", o.shell)
	}
	connector, err := os.ReadFile(o.connector)
	if err != nil {
		return nil, err
	}
	if _, err := pesig.ParseBytes(connector); err != nil {
	...
	// The connector first: the window looks for it by name in the
	// unpacked payload, and it finds ffmpeg.exe and units\ beside itself.
	entries = append(entries, payload.Entry{Name: "masseuse-camlink.exe", Data: connector})
	...
	packed, err := payload.Append(shell, o.version, entries)
	if err != nil {
		return nil, err
	}
	...
	// window once the payload is taken off.
	...
	if !bytes.Equal(bare, shell) {
		return nil, errors.New("the package minus its payload is not the window")
	}
	shellSum, connectorSum := sha256.Sum256(shell), sha256.Sum256(connector)
	return &report{size: int64(len(written)), id: r.Info.ID(), shellSHA256: fmt.Sprintf("%x", shellSum), connectorSHA256: fmt.Sprintf("%x", connectorSum), files: r.Info.Manifest.Files}, nil
}
