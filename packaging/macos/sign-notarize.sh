...
# The bundle is signed by sign-app.sh (the one signing plan: the programs
# inside first, ffmpeg with device.entitlements, each unit driver helper
# and the connector without, then the bundle, whose executable is the
# desktop window, with device.entitlements, all with the hardened runtime),
# then zipped for the notary service and stapled. The disk image is
...
case "$kind" in
  app)
    sh "$here/sign-app.sh" -i "$APPLE_CERT_SHA1" -k "$keychain" "$target"
